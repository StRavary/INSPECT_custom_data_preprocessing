...
> **NOTE — Pre-Fine-Tuning Baseline:** Confirmed via the official INSPECT GitHub repository (`som-shahlab/INSPECT_public`) that the `resnetv2_ct` checkpoint is **not** fine-tuned on the RSPECT PE detection dataset. The INSPECT paper's imaging pipeline requires the ResNetV2 to first be fine-tuned on RSPECT (publicly available via the AWS Open Data Registry at no egress cost, which can be downloaded using `Custom/0d_download_rspect_images.py`) before being used as a slice encoder. The current embeddings therefore represent a **pre-fine-tuning baseline** — the ResNet encodes general chest CT anatomy but has not been explicitly trained to identify PE-specific features such as filling defects, RV/LV strain patterns, or clot burden. The full replication pipeline is:
