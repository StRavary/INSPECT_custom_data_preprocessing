...
## 11-14. Image Vectorization & Baseline Ablation Pipeline (Legacy Steps Removed)

> **Note:** The legacy ResNetV2-101x3 pre-trained CTPA vectorization, PCA compression, vector ingestion dataset, and initial t-SNE ablation validation scripts (`Custom/5_process_ctpa.py`, `6_analyze_vectors.py`, `7_compress_vectors.py`, `8_vector_ingestion.py`, `tsne_compressed_vectors.py`, and `check_compressed_similarity.py`) have been removed from the repository. The image analysis pipeline has been replaced by a modified CTPA image analysis pipeline.
