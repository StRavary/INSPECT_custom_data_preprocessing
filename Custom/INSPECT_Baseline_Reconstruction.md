...
* **Hardware/CUDA 12+ Incompatibility (Blackwell GPUs):** The legacy Python 3.10 environment relies on older JAX/jaxlib wheels pinned to CUDA 11, which do not support modern GPUs with Compute Capability 12.0+ (like the Blackwell 50-series). Upgrading JAX to CUDA 12 breaks the `numpy` 1.x C-API binding required by `femr`.
  * **Solution:** To execute MOTOR/CLMBR GPU training on modern architectures without upgrading the python packages, the CUDA 12 `ptxas` (assembler) must be injected into the XLA compilation pipeline manually. Download it via `pip download nvidia-cuda-nvcc-cu12==12.5.82 --no-deps` and unzip it. Then, prepend it to your execution environment using `XLA_FLAGS="--xla_gpu_cuda_data_dir=/path/to/nvidia/cuda_nvcc"` and `PATH="/path/to/nvidia/cuda_nvcc/bin:$PATH"` prior to running GPU workloads.
* The script's hardcoded paths attempted to load the Foundation Model weights from internal `/share/pi/` servers. While the MOTOR model is actively hosted on Hugging Face (`StanfordShahLab/motor-t-base`), access requires formal approval.
...
Executing `clmbr_train_linear_probe` on a Blackwell GPU (Compute Capability 12.0, e.g., RTX 5090) required resolving three independent failure modes that each manifested as a silent hang. This section documents the complete diagnosis and the three-part fix that was required to successfully run GPU training.

**Root Cause Summary**

The `femr` library contains a custom CUDA C++ extension for local attention. This kernel was compiled for older GPU architectures and physically deadlocks the streaming multiprocessors (SMs) on Blackwell hardware. Additionally, the default XLA PTX assembler (`ptxas`) runs aggressive register allocation at `-O3`, which enters a non-linear optimization loop on the large attention graph generated by the fallback path. These two issues stack: bypassing the C++ kernel produces a larger graph, which then hangs the assembler.

**Fix 1 — Float32 Cast in `transformer.py` (Bypass the C++ Kernel)**

The femr attention kernel is invoked when query/key/value tensors are in their default dtype (typically `bfloat16`). Casting them to `float32` before the attention call routes the computation through JAX's native cuBLAS fallback, completely bypassing the deadlocking C++ kernel.

The patch is applied at line 129 of the installed femr library:
```
/path/to/venv_legacy/lib/python3.10/site-packages/femr/models/transformer.py
```

The patched line reads:
```python
attn_f32 = femr.jax.local_attention(q.astype(jnp.float32), k.astype(jnp.float32), v.astype(jnp.float32), length_mask, self.config["attention_width"])
```

> **NOTE:** This patch is applied directly to the installed `femr` package inside the virtualenv, not to any file in this repository. It must be re-applied whenever the virtualenv is rebuilt or `femr` is reinstalled.

**Fix 2 — `ptxas` Wrapper Script (Bypass the Assembler Optimization Loop)**

Because JAX 0.4.8's XLA backend does not support passing custom flags to `ptxas` via environment variables, a bash wrapper script intercepts the XLA call to the system assembler and forcefully injects the `-O0` flag, disabling the optimization that causes the hang.

Create the wrapper at a location inside the virtualenv's `bin/` directory (so it is guaranteed to be on `PATH` for any script launched through the venv):

```bash
cat > /path/to/venv_legacy/bin/ptxas << 'EOF'
#!/bin/bash
exec /usr/bin/ptxas -O0 "$@"
EOF
chmod +x /path/to/venv_legacy/bin/ptxas
```

Verify `/usr/bin/ptxas` is the correct path to the real assembler on your system (`which ptxas` before creating the wrapper, or check `/usr/local/cuda/bin/ptxas`). The wrapper must call the real binary — not itself.

> **CRITICAL — PATH at launch time:** The wrapper must be on the `PATH` of the process that runs the training script, not just the shell where you create it. Placing it inside the venv `bin/` directory (as above) guarantees this. If you place it elsewhere (e.g., `~/ptxas`), you must prepend that directory to `PATH` in the same command that launches the training script. Verify with: `cat /proc/<PID>/environ | tr '\0' '\n' | grep PATH`.

**Fix 3 — Unset `JAX_PLATFORMS` (Prevent Silent CPU Fallback)**

JAX silently falls back to CPU if the environment variable `JAX_PLATFORMS=cpu` is set. This causes the script to hang with 0% GPU utilization and no VRAM usage — indistinguishable from a data loading stall. This variable was set during earlier troubleshooting and must be cleared before launching any GPU workload.

```bash
unset JAX_PLATFORMS
# Verify JAX sees the GPU before launching:
python -c "import jax; print(jax.devices())"
# Expected: [GpuDevice(id=0, process_index=0)]
```

Check whether it is persisted in any shell config file and remove it:
```bash
...
If the script hangs, run `watch -n2 nvidia-smi` and interpret the GPU state:

| GPU Util | VRAM Usage | Meaning |
|---|---|---|
| 0% | 0 MB | JAX is on CPU (`JAX_PLATFORMS=cpu`?) or still loading data |
| 0% | >0 MB | XLA is compiling (ptxas is running) |
| 100% | >0 MB | Kernel is executing — if no progress, C++ kernel deadlock |
| 0% | 0 MB (stalled >15 min) | ptxas wrapper not on PATH; assembler stuck at `-O3` |

To distinguish data loading from ptxas compilation at 0% GPU / 0 VRAM, monitor the wrapper log:
```bash
# Add a log line to the wrapper to confirm it is being called:
echo 'echo "[ptxas-wrapper] $@" >> /tmp/ptxas_wrapper.log' >> /path/to/venv_legacy/bin/ptxas
tail -f /tmp/ptxas_wrapper.log
```

**Expected Timeline After a Clean Launch**
